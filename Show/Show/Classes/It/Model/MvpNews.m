...
//转模型类方法
+(instancetype)mvpNewsWithDict:(NSDictionary *)dict{
...
//1.创建单例加载
    ...
    //2.根据字典的keyEnumerator.nextObject拿到键名
            NSString *keyName = result.keyEnumerator.nextObject;
    //3.通过键名取得字典数组
            NSArray *dictArray = result[keyName];
    //4.遍历--转模型
            ...
            [dictArray enumerateObjectsUsingBlock:^(NSDictionary *dict, NSUInteger idx, BOOL * _Nonnull stop) {
                MvpNews *news = [MvpNews mvpNewsWithDict:dict];
                //添加
                [newsList addObject:news];
            }];
        //5.通过block,返回我们的模型数组
            if (finishedBlock) {
                finishedBlock(newsList.copy);
            }
        }
    }];
